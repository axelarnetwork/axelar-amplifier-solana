use solana_axelar_gas_service_legacy::events::GasCollectedEvent;
...
async fn test_receive_funds() {
    ...
    let ix = solana_axelar_gas_service_legacy::instructions::collect_fees_instruction(
...
async fn test_refund_native_fails_if_not_signed_by_authority() {
    ...
    let receiver = Keypair::new();
    let sol_amount = 1_000_000;
    let mut ix = solana_axelar_gas_service_legacy::instructions::collect_fees_instruction(
        &gas_utils.operator.pubkey(),
        &receiver.pubkey(),
        sol_amount,
    )
