...
fn execute_interchain_transfer_success() {
    ...
    let deploy_result = fixture.approve_and_execute(
        &deploy_message,
        deploy_encoded_payload.clone(),
        deployer_execute_accounts,
    );
    assert!(
        deploy_result.is_ok(),
    ...
    let transfer_result = fixture.approve_and_execute(
        &transfer_message,
...
fn reject_execute_interchain_transfer_with_zero_amount() {
    ...
    let transfer_result = fixture.approve_and_execute_with_checks(
        &transfer_message,
        transfer_encoded_payload.clone(),
        transfer_execute_accounts.clone(),
        Some(vec![checks]),
...
fn reject_execute_interchain_transfer_with_invalid_token_id() {
    ...
    let token_id = [2u8; 32]; // Not registered
