...
impl ItsTestHarness {
    ...
    pub fn ensure_gateway_initialized(&mut self) {
        ...
        self.gateway.verifier_set_leaves = verifier_leaves.clone();
    ...
    pub fn ensure_approved_incoming_messages(&mut self, messages: &[Message]) {
        let GatewayConfig {
            domain_separator, ..
        } = self
    ...
    pub fn ensure_trusted_chain(&mut self, trusted_chain_name: &str) {
        ...
        assert_eq!(its.trusted_chains.len(), 1);
        assert!(its.trusted_chains.iter().any(|x| x == trusted_chain_name));
    }
    ...
    pub fn execute_gmp(
        ...
        let incoming_message_pda =
            solana_axelar_gateway::IncomingMessage::find_pda(&payload_hash).0;
...
// fn get_ata_2022(wallet: &Pubkey, mint: &Pubkey) -> Pubkey {
//     get_associated_token_address_with_program_id(wallet, mint, &spl_token_2022::ID)
// }

// fn get_ata_legacy(wallet: &Pubkey, mint: &Pubkey) -> Pubkey {
...
fn test_execute_interchain_transfer() {
    ...
    let destination_address = receiver.to_bytes().to_vec();

    let transfer_amount = 1_000_000u64;
    let source_address_bytes = b"ethereum_address_123";

    let transfer_payload = interchain_token_transfer_gmp::InterchainTransfer {
        selector: InterchainTransfer::MESSAGE_TYPE_ID_UINT,
        token_id: alloy_primitives::FixedBytes::from(token_id),
        source_address: alloy_primitives::Bytes::from(source_address_bytes),
        ...
        data: alloy_primitives::Bytes::new(), // Empty data for simple token transfer
    };

    let transfer_payload_wrapped = ReceiveFromHub {
    ...
    let transfer_gmp_payload = GMPPayload::ReceiveFromHub(transfer_payload_wrapped);

    its_harness.execute_gmp(token_id, source_chain, receiver, transfer_gmp_payload);
