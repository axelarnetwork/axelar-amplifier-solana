...
use anchor_lang::{prelude::thiserror, AnchorSerialize};
...
impl RelayerDiscovery {
    ...
    pub fn convert_account(
        ...
        match account {
            ...
            RelayerAccount::MessagePayload => match self.payload_pda {
                Some(payload_pda) => Ok(AccountMeta {
                    pubkey: payload_pda,
                    is_signer: false,
                    is_writable: false,
                }),
            ...
            RelayerAccount::Payer(lamports) => {
                let payer =
                    self.payers.iter().enumerate().find(|(index, payer)| {
                        payer.1 == *lamports && !used_payers.contains(index)
                    });
                match payer {
                    Some((index, (pubkey, _))) => {
    ...
    pub fn convert_data(&self, data: &RelayerData) -> Result<Vec<u8>, ConvertError> {
        match data {
            ...
            RelayerData::Message => {
                ...
                self.message
                    .serialize(&mut bytes)
                    .map_err(|_| ConvertError::FailedMessageSerialization)?;
                Ok(bytes)
            }
            RelayerData::Payload => {
                let mut bytes = Vec::with_capacity(size_of::<Message>());
            ...
            RelayerData::PayloadRaw => Ok(self.payload.clone()),
            RelayerData::CommandId => Ok(Vec::from(self.message.command_id())),
        }
    }
    ...
    pub fn add_payer(&mut self, payer: Pubkey, amount: u64) {
        self.payers.push((payer, amount));
    }

    /// Add a payload pda to the relayer discovery.
    pub fn add_payload_pda(&mut self, payload_pda: Pubkey) {
...
pub fn executable_relayer_accounts(
    command_id: &[u8; 32],
