...
use crate::gateway::GatewayHarnessInfo;
...
impl ItsTestHarness {
    ...
    pub fn ensure_gateway_initialized(&mut self) {
        ...
        // Generate signers
        let (secret_key_1, compressed_pubkey_1) = crate::gateway::generate_random_signer();
        let (secret_key_2, compressed_pubkey_2) = crate::gateway::generate_random_signer();

        self.gateway.signers = vec![secret_key_1, secret_key_2];

        let previous_verifier_retention = solana_axelar_std::U256::from(5u64);
        ...
        let verifier_leaves = vec![
            solana_axelar_std::VerifierSetLeaf {
                nonce: 0,
                quorum: quorum_threshold,
                signer_pubkey: solana_axelar_std::PublicKey(compressed_pubkey_1),
                signer_weight: 50,
                position: 0,
                set_size: 2,
                domain_separator,
            ...
            solana_axelar_std::VerifierSetLeaf {
                nonce: 0,
                quorum: quorum_threshold,
                signer_pubkey: solana_axelar_std::PublicKey(compressed_pubkey_2),
                signer_weight: 50,
                position: 1,
                set_size: 2,
                domain_separator,
        ...
        let verifier_leaf_hashes: Vec<[u8; 32]> = verifier_leaves
            .iter()
            .map(solana_axelar_std::hasher::LeafHash::hash)
            .collect();
        let verifier_merkle_tree =
            solana_axelar_std::MerkleTree::from_leaves(&verifier_leaf_hashes);
        ...
        let initial_verifier_set = solana_axelar_gateway::state::config::InitialVerifierSet {
        ...
        let params = solana_axelar_gateway::state::config::InitializeConfigParams {
            domain_separator,
    ...
    pub fn ensure_approved_incoming_messages(&self, messages: &[CrossChainMessage]) {
        let solana_axelar_gateway::GatewayConfig {
            domain_separator, ..
        } = self
        ...
        // Merkle tree
        let message_leaves: Vec<solana_axelar_std::MessageLeaf> = messages
            .iter()
            .enumerate()
            .map(|(i, msg)| solana_axelar_std::MessageLeaf {
                message: msg.clone(),
                position: i as u16,
        ...
        let message_merkle_tree = solana_axelar_std::MerkleTree::from_leaves(&message_leaf_hashes);

        let payload_merkle_root = message_merkle_tree.root().unwrap();

        let solana_axelar_gateway::VerifierSetTracker {
            verifier_set_hash, ..
        } = self
        ...
        let verifier_infos = self
            ...
            .enumerate()
            .map(|(idx, (sk, l))| {
                crate::gateway::create_verifier_info(
                    sk,
                    payload_merkle_root,
                    l,
                    idx,
